...
| iSH (iPhone / iPad) | `apk add curl clang libc-dev` first, then run the installer |
...
> **iSH note:** iSH (Alpine Linux) does not ship `curl` or system headers by default.
> Run this first, then proceed with Step 1:
> ```sh
> apk add curl clang libc-dev
> ```
> `libc-dev` provides `pthread.h` and other headers required to compile.
> The installer also runs this automatically once curl is available.
...
or C system headers by default. Run this once before the installer:
